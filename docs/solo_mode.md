...
残件 (次にやるなら): 読み出しレートが 40/s (設計 100Hz)。トランザクション数が律速で、
`aux` を 2 周期に 1 本へ分散する派生元の実装は入っているので、次は quat の 4 ワード読みを
1 トランザクションに戻せるか (8B 一括はこの個体で化ける) か、I2C を 200kHz 程度で試す。
描画が約 49fps なので、40/s でも毎フレーム更新に近く体感上の影響は小さい。
