...
public:
     ...
     */
    bool begin(IMUManager& imu, SoundManager* sound = nullptr);
...
private:
    ...
    /**
     * @brief 検出イベントをログ出力する
     * @note solo 構成では外部への通知先が無いため、検出結果は Serial ログに残すだけ。
     *       LED/サウンドのフィードバックは setOnModeChange / setOnSelection で受ける。
     */
